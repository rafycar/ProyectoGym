...
async function selectExercises(filters) {
  // recoger filtros
  const { name, typology, muscles } = filters;
  ...
  // consulta: segementos q se van añadiendo con cada filtro (nuevo where)
  ...
  // devolver exercises (array con todos los objetos exercice q pasen los filtros)
  return exercises[0];
}
